...
public class ViewPagerIndicator extends LinearLayout {
    ...
    private static final int COLOR_TEXT_HIGHLIGHT = Color.parseColor("#FF4081");
    ...
    public void scroll(int position, float positionOffset) {
        ...
        mInitTranslationX = (int) (position * tabWidth + positionOffset * tabWidth + (tabWidth - mTriangleWidth) / 2);

        // 容器移动，当tab处于可见最后一个position时
        //Log.e("TAG", "tabWidth=" + tabWidth + ", visibleCount=" + mTableVisibleCount + ", position=" + position);
        if (getChildCount() > mTableVisibleCount && positionOffset > 0 && position >= mTableVisibleCount - 1) {
            Log.e("TAG", "scroll by x=" + (tabWidth * positionOffset));
            this.scrollTo((int) ((position + 1 - mTableVisibleCount) * tabWidth + tabWidth * positionOffset), 0);
        }
